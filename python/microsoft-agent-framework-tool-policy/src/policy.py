"""Local offline policy engine for the microsoft-agent-framework example.
...
The ``MicrosoftAgentFrameworkAdapter`` patches
``agent_framework.FunctionTool.invoke`` — the single async coroutine through
which every function tool executes — and routes every invocation through
``check_tool_start`` below. A ``deny`` (or an unapproved ``pending``) decision
raises ``PolicyViolationError`` before the tool body runs.
...
class LocalPolicyEngine:
    ...
    The Microsoft Agent Framework adapter calls ``check_tool_start`` (and, for
    pending tools, ``wait_for_tool_approval``) as ``async`` hooks. Both return
    decision dicts in the gateway wire format:
    ...
    async def wait_for_tool_approval(self, **kwargs: Any) -> dict[str, str]:
        ...
        return {
            "status": "deny",
            "reason": (
                f"Tool '{tool_name}' requires approval, but no approver is available in offline mode."
            ),
        }

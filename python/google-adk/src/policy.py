"""Local offline policy engine for the google-adk-governed-agent example.
...
The ``GoogleADKAdapter`` patches ``google.adk.tools.BaseTool.run_async`` and
routes every tool invocation through ``check_tool_start`` below. A ``deny`` (or
an unapproved ``pending``) decision raises ``PolicyViolationError`` before the
tool body runs.
...
class LocalPolicyEngine:
    ...
    The Google ADK adapter calls ``check_tool_start`` (and, for pending tools,
    ``wait_for_tool_approval``) as ``async`` hooks. Both return decision dicts
    in the gateway wire format: ``{"status": "allow" | "deny" | "pending"}``.
    """
